...
async function ERC1820Registry (funder) {
  ...
  if ((await web3.bnb.getCode(ERC1820_REGISTRY_ADDRESS)).length > '0x0'.length) {
  ...
  await web3.bnb.sendSignedTransaction(ERC1820_REGISTRY_DEPLOY_TX);
...
async function getDeployedERC1820Registry () {
  ...
  if (config.abstraction === 'truffle') {
    ...
    return new web3.bnb.Contract(ERC1820_REGISTRY_ABI, ERC1820_REGISTRY_ADDRESS);
